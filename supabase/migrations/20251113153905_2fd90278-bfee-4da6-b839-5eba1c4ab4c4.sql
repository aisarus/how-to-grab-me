...
-- Create storage policies for data-room-documents bucket
...
TO authenticated
WITH CHECK (
  bucket_id = 'data-room-documents' AND
  auth.uid()::text = (storage.foldername(name))[1]
);

-- Policy for downloading (SELECT)
...
CREATE POLICY "Users can update in their own folder"
ON storage.objects
FOR UPDATE
TO authenticated
USING (
