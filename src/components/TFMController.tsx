...
export const TFMController = () => {
  ...
  const [smartQueueResult, setSmartQueueResult] = useState<SmartQueueResult & { recommendation?: string } | null>(null);
  ...
  // Check Smart Queue when prompt changes
  useEffect(() => {
    const checkSmartQueue = async () => {
      if (!prompt.trim() || prompt.length < 20) {
      ...
      try {
        const { data, error } = await supabase.functions.invoke('smart-queue-check', {
          body: { prompt }
        });
